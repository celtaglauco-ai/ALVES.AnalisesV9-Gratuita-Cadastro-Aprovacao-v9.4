...
export type Session={id:string;name:string;role:"admin"|"user";exp:number;ver?:number};
...
export function validAdmin(user:string,password:string){const configuredUser=process.env.ADMIN_USER?.trim().toLowerCase()||ADMIN_USER,configuredHash=process.env.ADMIN_PASSWORD_HASH?.trim()||ADMIN_HASH;return user===configuredUser&&verifyPassword(password,configuredHash)}
export async function createSession(data:{id:string;name:string;role:"admin"|"user";ver?:number},remember=false){const duration=remember?30*24*60*60:8*60*60,payload=Buffer.from(JSON.stringify({...data,exp:Date.now()+duration*1000})).toString("base64url");(await cookies()).set(COOKIE,`${payload}.${sign(payload)}`,{httpOnly:true,secure:process.env.NODE_ENV==="production",sameSite:"lax",path:"/",maxAge:duration})}
...
export async function isAuthorized(){const s=await getSession();if(!s)return false;if(s.role==="admin")return true;try{await initDb();const {rows}=await pool.query("SELECT status,session_version FROM users WHERE id=$1 LIMIT 1",[s.id]);return rows[0]?.status==="approved"&&Number(rows[0]?.session_version||1)===Number(s.ver||1)}catch{return false}}
