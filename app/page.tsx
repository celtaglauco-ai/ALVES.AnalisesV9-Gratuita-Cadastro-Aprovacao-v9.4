...
export default function Home() {
  ...
  const analyzePreBot=()=>{
    ...
    const leaguesSelected=!!leagueId&&!!awayLeagueId,
      ...
      selectedPicks=availablePicks.filter(x=>x.chance>=65&&x.overall>=60&&x.recent>=60).sort((x,y)=>y.score-x.score),reasons:string[]=[];
    ...
    if(!selectedPicks.length)reasons.push("Nenhum mercado atingiu simultaneamente os mínimos de histórico geral, forma recente e estabilidade.");
    const makeSuggestion=(id:PreBotSuggestion["id"],title:PreBotSuggestion["title"],count:number,exposure:PreBotSuggestion["exposure"],recommended:boolean):PreBotSuggestion|null=>{
      ...
      recommendedId=balancedStrong?"balanced":"conservative",
