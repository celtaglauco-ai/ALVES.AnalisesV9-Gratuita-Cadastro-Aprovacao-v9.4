...
type AnalysisHistory={id:string;mode:"pre"|"prebot"|"live";league_id:string;home:string;away:string;snapshot:{awayLeagueId?:string;homeStats?:HistoryStats;awayStats?:HistoryStats;probabilities?:{goals:number;btts:number;corners:number;cards:number};live?:Record<string,number>|null};created_at:number;market?:string;confidence?:number;result_status?:"pending"|"hit"|"miss";result_note?:string};
type PersonalPerformance={total:number;hits:number;misses:number;pending:number;accuracy:number};
type PreBotResult={approved:boolean;markets:string[];confidence:number;risk:"Baixo"|"Médio"|"Alto";evidence:string[];reason:string};
...
export default function Home() {
  ...
  const loadPrivateHistory=async()=>{if(admin)return;setHistoryLoading(true);try{const r=await fetch("/api/user/data",{cache:"no-store"}),d=await r.json();if(r.ok){setAnalysisHistory(d.history||[]);setPersonalPerformance(d.performance||{total:0,hits:0,misses:0,pending:0,accuracy:0})}}finally{setHistoryLoading(false)}};
  useEffect(() => {
    fetch("/api/auth/me")
      .then((r) => r.json())
      .then((d) => {
  ...
  const savePreBotPrediction=async()=>{
    if(admin)return setNotice("O histórico de previsões pertence aos usuários. Entre com uma conta de usuário para salvar.");
          ...
          {(tab === "pre" || (tab === "live" && analyzed)) &&
            (!ready ? (
              ...
              <>
                ...
                {!admin&&<section className="panel personal-performance"><div className="panel-head"><i className="green">✓</i><div><h3>Meu desempenho pessoal</h3><p>Somente você visualiza estes resultados</p></div></div><div><article><small>ANÁLISES</small><b>{personalPerformance.total}</b></article><article><small>TAXA DE ACERTO</small><b>{personalPerformance.accuracy}%</b></article><article><small>ACERTOS</small><b>{personalPerformance.hits}</b></article><article><small>ERROS</small><b>{personalPerformance.misses}</b></article><article><small>AGUARDANDO</small><b>{personalPerformance.pending}</b></article></div><p>A taxa considera somente análises que tiveram o resultado confirmado pelo administrador.</p></section>}
                {!admin&&<section className="panel private-history-panel"><div className="panel-head"><i className="purple">◷</i><div><h3>Meu histórico de análises</h3><p>Privado e separado dos demais usuários</p></div><button onClick={loadPrivateHistory} disabled={historyLoading}>{historyLoading?"Atualizando...":"Atualizar"}</button></div><div className="private-history-list">{analysisHistory.slice(0,12).map(item=><article key={item.id}><span><small>{item.mode==="live"?"AO VIVO":item.mode==="prebot"?"BOT PRÉ-LIVE":"PRÉ-JOGO"} • {item.market||"ANÁLISE GERAL"}</small><b>{item.home} × {item.away}</b><time>{new Date(Number(item.created_at)).toLocaleString("pt-BR")}</time></span><em className={`history-result result-${item.result_status||"pending"}`}>{item.result_status==="hit"?"✓ ACERTO":item.result_status==="miss"?"× ERRO":"⌛ AGUARDANDO"}</em><div><button onClick={()=>reopenHistory(item)}>Abrir</button><button className="danger" onClick={()=>deleteHistory(item.id)}>Excluir</button></div></article>)}{!analysisHistory.length&&<p className="nodata">Você ainda não salvou nenhuma análise.</p>}</div></section>}
