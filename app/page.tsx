...
export default function Home() {
  ...
  const fetchLiveGames=async()=>{setLiveApiLoading(true);setLiveApiInfo("Consultando jogos de hoje...");try{const r=await fetch("/api/live",{cache:"no-store"}),d=await r.json();if(d.available){setLiveApiGames(d.games||[]);setLiveApiInfo(d.games?.length?`${d.games.length} jogos de hoje nas ligas cadastradas • cota restante: ${d.remaining||"—"}`:(d.reason||"Nenhum jogo de hoje foi encontrado nas ligas cadastradas."))}else setLiveApiInfo(d.reason||"API indisponível")}finally{setLiveApiLoading(false)}};
