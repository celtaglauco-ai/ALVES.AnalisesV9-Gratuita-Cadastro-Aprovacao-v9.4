...
export async function POST(req:Request){
 ...
 const updated=results.filter(x=>x.ok).length,failed=results.filter(x=>!x.ok).length;
 return NextResponse.json({ok:updated>0,updated,failed,results},{status:updated>0?200:502});
}
