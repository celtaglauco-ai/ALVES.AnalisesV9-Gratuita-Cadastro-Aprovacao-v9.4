...
export async function GET(req:Request){
 ...
 const url=new URL(req.url),id=url.searchParams.get("leagueId")||"",force=url.searchParams.get("refresh")==="1",key=process.env.FOOTBALL_DATA_TOKEN;
 if(!key)return NextResponse.json({available:false,reason:"Football-Data.org ainda não configurada."});
